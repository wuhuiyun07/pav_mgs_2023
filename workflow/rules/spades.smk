...
rule all_spades:
    input: expand("reports/spades.3.15/{sample}/spades.log", sample = SAMPLES)
...
rule metaspades:
    ...
    output:
        dir = directory("results/spades.3.15/{sample}"),
        contigs="results/spades.3.15/{sample}/contigs.fasta",
    params:
        k= "auto",
    log:
        "reports/spades.3.15/{sample}/spades.log",     
    container:
